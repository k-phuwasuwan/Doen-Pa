...
- `PlaceGallery`: show one large image at a time, using the user's own TravelRecord photos for that place. If no photo exists, show a plain gray placeholder. For multiple photos, overlay Liquid Glass previous/next buttons and a position counter on the image; do not render a thumbnail row. Clicking the image opens the current photo in the full-screen viewer. Do not scale gallery images on hover.
