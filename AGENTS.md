...
- Stats and Profile derive personal counts from the current user's records. Stats shows travel, province, national park, place type, region, and photo counts; the badge teaser and collection are deferred. The national-park progress card sits below province progress and counts the current catalog's 11 destinations as the total, with one unique visited place counting once toward that total regardless of its display category. Profile uses a real photo count in place of the former badge count. Profile photo tiles link to Place Details with `from=profile`.
