...
- Stats and Profile derive personal counts from the current user's records. Stats shows travel, province, national park, place type, region, and photo counts; the badge teaser and collection are deferred. The national-park progress card sits below province progress and uses the number of `national_park` places in the local catalog as its denominator, not a nationwide total. Profile uses a real photo count in place of the former badge count. Profile photo tiles link to Place Details with `from=profile`.
